 ...
 * Top KPI Cards (Stylish, compact, equal height, gap-1)
 */
...
import type { IntegrationsKpis } from "../data/types";

interface IntegrationsKpiCardsProps {
  kpis: IntegrationsKpis;
  activeFilter?: string;
  onSelectFilter?: (filterKey: string) => void;
  className?: string;
}

export function IntegrationsKpiCards({
  kpis,
  activeFilter,
  onSelectFilter,
  className,
}: IntegrationsKpiCardsProps) {
  const cards = [
    {
      id: "total_providers",
      label: "PROVIDERS",
  ...
  return (
    <div
      className={cn(
        "grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-4 xl:grid-cols-8 gap-1 items-stretch",
        className
      )}
    >
      {cards.map((c) => {
        ...
        return (
          ...
          >
            ...
            <div
              className={cn(
                "text-xl font-extrabold tracking-tight leading-tight my-0.5",
                c.valueColor
              )}
            >
              {c.value}
