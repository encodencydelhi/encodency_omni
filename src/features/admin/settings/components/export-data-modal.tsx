...
import { Download, Loader2, Check, X, FileArchive, ShieldAlert } from "lucide-react";
...
export function ExportDataModal({ open, onClose, onRequestExport }: ExportDataModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-3 bg-slate-900/50 backdrop-blur-xs animate-in fade-in duration-150">
      <div className="w-full max-w-lg bg-white rounded-2xl border border-[#E2E8F0] shadow-2xl p-3 space-y-2">
          ...
          <>
              ...
              <div className="space-y-1 max-h-[240px] overflow-y-auto pr-1">
                {AVAILABLE_CATEGORIES.map((cat) => {
                  ...
                  return (
                    <label
                      key={cat.id}
                      className="flex items-center justify-between p-2 rounded-lg border border-[#E2E8F0] hover:bg-slate-50 cursor-pointer transition-colors"
                    >
                      <div className="pr-2">
                        <div className="text-[11.5px] font-semibold text-[#111C3A]">{cat.label}</div>
                        <div className="text-[9.5px] text-[#64748B]">{cat.desc}</div>
                      </div>
                      <input
                        type="checkbox"
                        checked={isChecked}
            ...
            <div className="flex items-center justify-end gap-2 pt-2 border-t border-[#F1F5F9]">
              <button
                type="button"
                onClick={onClose}
                className="px-3 py-1.5 rounded-lg border border-[#CBD5E1] text-[11px] font-semibold text-[#475569] hover:bg-slate-50 cursor-pointer"
              >
                Cancel
              </button>
              <button
        ...
        {status === "preparing" && (
          <div className="py-3 text-center space-y-2">
            ...
            <div>
              ...
              <p className="text-[10px] text-[#64748B]">Packaging selected database records and configurations.</p>
            </div>
            ...
            <span className="text-[9.5px] font-mono text-slate-500">{progress}% complete</span>
          </div>
        )}

        {status === "ready" && (
          <div className="py-3 text-center space-y-2">
            <div className="size-8 rounded-full bg-emerald-50 text-emerald-600 flex items-center justify-center mx-auto border border-emerald-200">
