...
export default function IndiaMap({ data }: { data: StateData[] }) {
  ...
  return (
    <div ref={containerRef} className="relative h-[180px] w-full">
      {tooltipData && (
        <div
          className="pointer-events-none absolute z-50 rounded-sm border border-[#e2e8f0] bg-white px-3 py-2 shadow-lg"
        ...
        >
          ...
          {tooltipData.value && (
            <div className="flex items-center gap-2 text-[11px]">
              ...
              <span className="rounded-sm bg-[#eef2ff] px-1.5 py-0.5 text-[11px] font-semibold text-[#4f46e5]">{tooltipData.pct}%</span>
            </div>
          )}
          {!tooltipData.value && (
