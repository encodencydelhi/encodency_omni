  ...
  return (
    <div className="overflow-hidden rounded-sm border border-[#DDE6F1] bg-white shadow-[0_1px_4px_rgb(15_23_42/0.05)]">
      <div className="flex items-center gap-3 border-b border-[#E7EDF5] p-3.5">
        <span className="grid size-9 shrink-0 place-items-center rounded-sm bg-[#FFE6EA] text-[#EB0711]">
        ...
        <button className="flex h-8 shrink-0 items-center gap-1.5 rounded-sm border border-[#DDE6F1] bg-white px-3 text-[11px] font-semibold text-[#155EEF]">
          ...
          <div className="grid grid-cols-2 gap-1.5 md:grid-cols-5">
            {[
              ...
              <button
                ...
                className={cn(
                  "flex h-[42px] items-center justify-center gap-1.5 rounded-sm border px-2 text-[12px] font-semibold leading-tight",
      ...
      <div className="grid gap-x-4 gap-y-3 border-b border-[#E7EDF5] p-3.5 xl:grid-cols-[1.05fr_.95fr_1fr]">
        <Field label="Age Range" required>
          <div className="px-1 pt-1">
            ...
            <span className="relative block h-1.5 rounded-sm bg-[#E7EDF5]">
              <i className="absolute inset-y-0 left-[22%] right-[34%] rounded-sm bg-[#155EEF]" />
              <i className="absolute left-[22%] top-1/2 size-4 -translate-x-1/2 -translate-y-1/2 rounded-sm border-2 border-white bg-[#155EEF] shadow" />
              <i className="absolute right-[34%] top-1/2 size-4 translate-x-1/2 -translate-y-1/2 rounded-sm border-2 border-white bg-[#155EEF] shadow" />
            </span>
          ...
          <div className="grid grid-cols-4 gap-1.5">
            {[
              ...
              <button
                ...
                className={cn(
                  "h-[56px] rounded-sm border text-center text-[12px] font-semibold",
      ...
      <div className="border-b border-[#E7EDF5] p-3.5">
        <div className="flex flex-wrap items-center gap-2 rounded-sm border border-[#DDE6F1] bg-white px-3 py-2">
          ...
          {["Environment", "River Conservation", "Sustainability", "Climate Action", "Volunteers", "NGOs", "Students"].map((tag, index) => (
            <span
              key={tag}
              className={cn(
                "rounded-sm px-3 py-1 text-[10.5px] font-semibold",
      ...
      <div className="grid gap-2 border-b border-[#E7EDF5] p-3.5 xl:grid-cols-4">
        <AudienceBox icon={Users} title="Custom Audience" caption="Upload existing audience data." on={true}>
          <button className="mt-3 flex h-10 w-full items-center justify-center gap-2 rounded-sm border border-dashed border-[#9BC2FF] bg-white text-[11px] font-semibold text-[#155EEF]">
      ...
      <div className="grid gap-2 p-3.5 xl:grid-cols-[.9fr_1fr]">
        <div className="flex items-center gap-3 rounded-sm border border-[#DDE6F1] bg-white p-3">
          <span className="grid size-10 shrink-0 place-items-center rounded-sm bg-[#EAF2FF] text-[#155EEF]">
            <BarIcon />
          </span>
          <div className="min-w-0 flex-1">
          ...
          <span className="inline-flex items-center gap-1 rounded-sm bg-[#DDF8EA] px-3 py-1 text-[10.5px] font-semibold text-[#078359]">
            <Check className="size-3" />
            Good reach
          </span>
        </div>

        <div className="grid gap-3 rounded-sm border border-[#DDE6F1] bg-white p-3 lg:grid-cols-[1fr_1fr]">
            ...
            <ul className="min-w-0 flex-1 space-y-1">
              {AGE_BREAKDOWN.map((slice) => (
                <li key={slice.name} className="flex items-center gap-1.5 text-[10px]">
                  <i className="size-2 rounded-sm" style={{ backgroundColor: slice.color }} />
            ...
            <div className="space-y-2">
              {GENDER_SPLIT.map((item) => (
                <div key={item.name} className="grid grid-cols-[74px_1fr_28px] items-center gap-2 text-[10px]">
                  ...
                  <span className="h-2 rounded-sm bg-[#E7EDF5]">
                    <i className="block h-full rounded-sm" style={{ width: `${item.value}%`, backgroundColor: item.color }} />
                  </span>
...
function AudienceBox({
  ...
  return (
    <div className="min-h-[150px] rounded-sm border border-[#DDE6F1] bg-white p-3">
      <div className="mb-2 flex items-start gap-2">
        <span className="grid size-8 shrink-0 place-items-center rounded-sm bg-[#F2EAFF] text-[#7C3AED]">
...
function MiniSelect({ label, value }: { label: string; value: string }) {
  return (
    <label className="mb-2 block">
      ...
      <span className="flex h-9 items-center rounded-sm border border-[#DDE6F1] px-2.5 text-[12px] font-semibold text-[#34415F]">
        {value}
...
function PieMiniIcon() {
  return (
    <span className="relative size-5 rounded-sm border-[5px] border-[#155EEF] border-r-[#9BC2FF] border-t-[#0AA673]" />
  );
}
