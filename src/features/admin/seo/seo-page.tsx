...
function Stat({ icon: Icon, label, value, danger }: { icon: typeof Gauge; label: string; value: string; danger?: boolean }) { return <div className="rounded-sm border border-[#DDE4ED] bg-white p-2.5 shadow-sm"><Icon className={cn("size-4", danger ? "text-[#EB0711]" : "text-[#168762]")} /><p className="mt-2 text-[7.5px] text-[#75829D]">{label}</p><p className="text-[17px] font-semibold text-[#172044]">{value}</p></div> }
function Panel({ title, children }: { title: string; children: React.ReactNode }) { return <section className="overflow-hidden rounded-sm border border-[#DDE4ED] bg-white shadow-sm"><h2 className="border-b border-[#E8EDF3] px-3 py-2.5 text-[10px] font-semibold text-[#172044]">{title}</h2>{children}</section> }
function Issue({ title, severity, pages }: { title: string; severity: string; pages: number }) { return <div className="flex items-center gap-2 border-b border-[#E8EDF3] px-3 py-2.5"><span className={cn("grid size-7 place-items-center rounded-sm", severity === "Critical" ? "bg-[#FFE8EA] text-[#D91521]" : "bg-[#FFF3DC] text-[#B27818]")}><AlertTriangle className="size-3.5" /></span><div className="flex-1"><p className="text-[8.5px] font-semibold text-[#24345D]">{title}</p><p className="text-[7.5px] text-[#75829D]">{pages} affected pages</p></div><span className="text-[7px] font-semibold text-[#EB0711]">{severity}</span></div> }
