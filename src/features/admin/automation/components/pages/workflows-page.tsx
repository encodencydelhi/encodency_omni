...
export function WorkflowsPage() {
  ...
  return (
    <div className="space-y-2 pb-12">
      ...
      <div className="bg-white rounded-xl border border-[#E2E8F0] shadow-sm p-4 flex flex-col">
        ...
        <div className="-mx-4 overflow-x-auto">
          <table className="w-full text-left text-[12px]">
            <thead>
              <tr className="border-b border-[#E2E8F0] text-[#64748B] whitespace-nowrap">
                <th className="pl-4 pr-2 py-2 w-[30px]"><input type="checkbox" className="rounded border-gray-300" /></th>
                <th className="px-3 py-2 font-medium">Workflow Name</th>
                <th className="px-3 py-2 font-medium">Trigger</th>
                <th className="px-3 py-2 font-medium">Channels</th>
                <th className="px-3 py-2 font-medium">Owner</th>
                <th className="px-3 py-2 font-medium">Last Updated</th>
                <th className="px-3 py-2 font-medium text-right">Runs (30d)</th>
            ...
            <tbody className="divide-y divide-[#F1F5F9]">
              {filteredWorkflows.map((wf) => (
                <tr key={wf.id} className="hover:bg-[#F8FAFC] transition-colors cursor-pointer whitespace-nowrap" onClick={() => setEditingWorkflow(wf)}>
                  ...
                  <td className="px-3 py-2.5 font-bold text-[#111C3A]">{wf.name}</td>
                  <td className="px-3 py-2.5 text-[#64748B]">{wf.trigger.label}</td>
                  ...
                  <td className="px-3 py-2.5">
                     <div className="flex items-center gap-2">
                       <div className="size-6 rounded-full bg-slate-100 border border-slate-200 flex items-center justify-center text-[9.5px] font-bold text-slate-700 shrink-0">
                  ...
                  <td className="px-3 py-2.5 text-[#64748B]">{wf.id === 'wf_1' ? '12 min ago' : '1 day ago'}</td>
                  <td className="px-3 py-2.5 text-[#111C3A] font-medium text-right">{wf.runs.toLocaleString()}</td>
                  ...
                  <td className="px-3 py-2.5" onClick={(e) => {
                    ...
                    toast.info(`Workflow "${wf.name}" set to ${nextStatus}`);
                  }}>
                    <button type="button" className="cursor-pointer" title="Click to toggle status">
                      <StatusChip status={wf.status} />
                    </button>
                  </td>
                  <td className="px-4 py-2.5 text-right" onClick={e => {
                    e.stopPropagation();
                    setEditingWorkflow(wf);
                  }}>
                    <button className="inline-flex items-center gap-1 text-[#2563EB] hover:text-[#1D4ED8] p-1 font-bold text-[11px] whitespace-nowrap cursor-pointer">
                      <span>Edit</span>
      ...
      {/* Full-Width Row: Recent Activity & Quick Templates */}
        ...
        <div className="bg-white rounded-xl border border-[#E2E8F0] shadow-sm p-4 flex flex-col">
          ...
          <div className="space-y-2 flex-1">
            {[
              ...
              { user: 'MS', name: 'Manish Sirohi', action: 'tested', target: 'Website Down Webhook Alert', time: '2 days ago', color: 'bg-indigo-500' },
              { user: 'RD', name: 'Ritu Dubey', action: 'scheduled', target: 'Abandoned Cart Winback', time: '3 days ago', color: 'bg-purple-500' },
              { user: 'VK', name: 'Vikram Kapur', action: 'published', target: 'Missed Call Callback v2', time: '3 days ago', color: 'bg-teal-500' },
            ].map((act, i) => (
      ...
      {/* Secondary Row: Categories & Trigger Sources */}
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-2">
        {/* Categories */}
        <div className="bg-white rounded-xl border border-[#E2E8F0] shadow-sm p-4 flex flex-col">
