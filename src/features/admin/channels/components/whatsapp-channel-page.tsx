    ...
    <div className="space-y-3 pb-8">
        ...
        <div className="flex items-center gap-2">
          <div className="flex items-center gap-3 border-r border-[#E8EDF3] pr-3 mr-1">
            <div className="text-right">
              <p className="text-[10px] font-semibold text-[#38444D]">WhatsApp Business API</p>
      ...
      <div className="flex items-center gap-6 border-b border-[#DDE4ED] px-2">
        {["Overview", "Campaigns", "Templates", "Conversations", "Contacts", "Automation", "Analytics", "Settings"].map((tab, i) => (
          <button key={tab} className={cn("pb-2 text-[10px] font-semibold", i === 0 ? "border-b-2 border-[#172044] text-[#172044]" : "text-[#71809D] hover:text-[#38444D]")}>{tab}</button>
        ))}
      </div>
      ...
      <div className="grid h-[240px] grid-cols-[1.5fr_1fr_1.2fr] gap-2">
        ...
        <Box title="Top Campaigns" action={<span className="text-[#EB0711] cursor-pointer">View All →</span>}>
          <div className="px-2">
            <div className="grid grid-cols-[1.5fr_.5fr_.6fr_.5fr_.5fr] py-1.5 text-[8.5px] font-semibold text-[#71809D]">
              <span>Campaign</span>
      ...
      <div className="grid h-[240px] grid-cols-[1.25fr_1.1fr_.55fr] gap-2">
        <Box title="Recent Campaigns" action={<span className="text-[#EB0711] cursor-pointer">View All →</span>}>
          <div className="px-2">
            <div className="grid grid-cols-[1.4fr_.6fr_.6fr_.6fr_.8fr] py-1.5 text-[8.5px] font-semibold text-[#71809D]">
              <span>Campaign</span>
              <span>Type</span>
              <span>Audience</span>
              <span>Status</span>
            ...
            {recentCampaigns.map((c) => (
              <div key={c.name} className="grid grid-cols-[1.4fr_.6fr_.6fr_.6fr_.8fr] items-center border-t border-[#EDF1F5] py-1.5 text-[9px]">
                <span className="truncate font-semibold text-[#172044]">{c.name}</span>
                <span><i className={cn("rounded px-1.5 py-0.5 text-[8px] font-semibold", c.type === "Marketing" ? "bg-[#FCE7F3] text-[#BE185D]" : "bg-[#DCFCE7] text-[#15803D]")}>{c.type}</i></span>
                <span className="text-[#52617D]">{c.audience}</span>
                <span><i className="rounded bg-[#DCFCE7] px-1.5 py-0.5 text-[8px] font-semibold text-[#15803D]">{c.status}</i></span>
          ...
          <div className="px-2">
            ...
            {messageTemplates.map((t) => (
              <div key={t.name} className="grid grid-cols-[1.2fr_.7fr_.5fr_.5fr] items-center border-t border-[#EDF1F5] py-2 text-[9px]">
                ...
                <span><i className={cn("rounded px-1.5 py-0.5 text-[8px] font-semibold", t.category === "Marketing" ? "bg-[#FCE7F3] text-[#BE185D]" : t.category === "Utility" ? "bg-[#DBEAFE] text-[#1D4ED8]" : "bg-[#F3E8FF] text-[#7E22CE]")}>{t.category}</i></span>
                <span className="text-[#52617D]">{t.lang}</span>
                <span><i className="rounded bg-[#DCFCE7] px-1.5 py-0.5 text-[8px] font-semibold text-[#15803D]">{t.status}</i></span>
              </div>
            ))}
          </div>
        </Box>
        ...
        <Box title="Quick Actions">
          <div className="p-1.5">
            {[
              ...
              <button key={i} className="flex w-full items-center gap-2.5 rounded px-2 py-1.5 text-left text-[9.5px] font-semibold text-[#172044] hover:bg-[#F8FAFC]">
      ...
      <div className="grid h-[240px] grid-cols-[1.2fr_1fr_.7fr] gap-2">
        <Box title="Recent Conversations" action={<span className="text-[#EB0711] cursor-pointer">View All →</span>}>
          <div className="px-2">
            <div className="grid grid-cols-[1.5fr_2fr_.5fr_.5fr] py-1.5 text-[8.5px] font-semibold text-[#71809D]">
              <span>Contact</span>
            ...
            {conversations.map((c, i) => (
              <div key={i} className="grid grid-cols-[1.5fr_2fr_.5fr_.5fr] items-center border-t border-[#EDF1F5] py-2 text-[9px]">
                <div className="flex items-center gap-2 overflow-hidden pr-2">
                  <span className={cn("grid size-[26px] shrink-0 place-items-center rounded-full font-semibold", c.color)}>{c.initial}</span>
                  <div className="min-w-0">
                    <p className="truncate font-semibold text-[#172044]">{c.name}</p>
                ...
                <span><i className={cn("rounded px-1.5 py-0.5 text-[8px] font-semibold", c.status === "Replied" ? "bg-[#DCFCE7] text-[#15803D]" : "bg-[#FEF9C3] text-[#A16207]")}>{c.status}</i></span>
              </div>
            ))}
          </div>
        </Box>
        ...
        <Box title="Audience Growth" action={<button className="flex items-center gap-1 text-[#38444D] text-[9px] font-semibold border rounded px-1.5 py-0.5"><ChevronDown className="size-2.5" /> Last 30 days</button>}>
          <div className="flex h-full flex-col p-3">
            ...
            <div className="mt-auto grid grid-cols-3 gap-2">
              <div><p className="text-[9px] text-[#71809D]">Total Contacts</p><p className="flex items-baseline gap-1.5"><b className="text-[18px] text-[#172044]">3,842</b><span className="text-[9px] font-semibold text-[#00A66A]">↑ 18%</span></p></div>
              <div><p className="text-[9px] text-[#71809D]">New Contacts</p><p className="flex items-baseline gap-1.5"><b className="text-[18px] text-[#172044]">624</b><span className="text-[9px] font-semibold text-[#00A66A]">↑ 32%</span></p></div>
              <div><p className="text-[9px] text-[#71809D]">Blocked</p><p className="flex items-baseline gap-1.5"><b className="text-[18px] text-[#172044]">48</b><span className="text-[9px] font-semibold text-[#EA111B]">↓ 12%</span></p></div>
            </div>
          </div>
        </Box>
