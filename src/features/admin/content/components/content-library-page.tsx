...
function AiAssistant({ prompt, setPrompt }: { prompt: string; setPrompt: (v: string) => void }) {
  return <div className="grid gap-5 lg:grid-cols-[1.15fr_.85fr]">
    <section className="rounded-sm border bg-white p-5 shadow-sm">
      <div className="mb-5 flex items-center gap-3"><span className="grid size-10 place-items-center rounded-sm bg-rose-50 text-[#e20611]"><Sparkles className="size-5" /></span><div><h2 className="text-[16px] font-semibold">AI Content Assistant</h2><p className="text-[12px] text-muted-foreground">Turn a simple idea into platform-ready content.</p></div></div>
      <label className="mb-2 block text-[12px] font-semibold">What would you like to create?</label>
      <textarea value={prompt} onChange={e => setPrompt(e.target.value)} rows={7} placeholder="Example: Create an inspiring Instagram post for our Clean Ganga volunteer drive..." spellCheck className="w-full resize-none rounded-sm border p-3 text-[13px] outline-none focus:border-[#e20611] focus:ring-1 focus:ring-[#e20611]" />
      <div className="mt-3 grid grid-cols-2 gap-3 sm:grid-cols-4">{["Social post", "Caption ideas", "Hashtags", "Rewrite text"].map(x => <button key={x} onClick={() => setPrompt(x)} className="rounded-sm border px-3 py-2 text-[11px] font-semibold hover:bg-gray-50">{x}</button>)}</div>
      <button className="mt-4 flex h-10 w-full items-center justify-center gap-2 rounded-sm bg-[#e20611] text-[13px] font-semibold text-white"><Sparkles className="size-4" />Generate Content</button>
    </section>
    <section className="rounded-sm border bg-white p-5 shadow-sm"><h2 className="text-[15px] font-semibold">Generated Content</h2><div className="mt-4 grid min-h-[260px] place-items-center rounded-sm border border-dashed bg-gray-50/60 p-8 text-center"><div><Sparkles className="mx-auto size-8 text-gray-300" /><p className="mt-3 text-[13px] font-semibold">Your AI result will appear here</p><p className="mt-1 text-[11px] text-muted-foreground">Add an idea and generate a caption, hashtags and CTA.</p></div></div></section>
  </div>;
}

function Templates() {
  return <div><div className="mb-4 flex items-center justify-between"><div><h2 className="text-[16px] font-semibold">Content Templates</h2><p className="text-[12px] text-muted-foreground">Start faster with reusable, channel-ready formats.</p></div><div className="flex h-9 w-64 items-center gap-2 rounded-sm border bg-white px-3"><Search className="size-4 text-muted-foreground" /><input className="w-full bg-transparent text-[12px] outline-none" placeholder="Search templates..." /></div></div><div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">{templates.map(([title, description, tag, color]) => <article key={title} className="group rounded-sm border bg-white p-4 shadow-sm transition hover:-translate-y-0.5 hover:shadow-md"><div className={cn("mb-4 grid h-28 place-items-center rounded-sm", color)}><ImageIcon className="size-8 opacity-70" /></div><span className="rounded bg-gray-100 px-2 py-1 text-[10px] font-semibold">{tag}</span><h3 className="mt-3 text-[14px] font-semibold">{title}</h3><p className="mt-1 text-[11px] text-muted-foreground">{description}</p><button className="mt-4 h-8 w-full rounded-sm border text-[11px] font-semibold text-[#e20611] hover:bg-rose-50">Use Template</button></article>)}</div></div>;
}

function Drafts() {
  // @ts-expect-error The compact draft-row map keeps its positional index for stable mock ordering.
