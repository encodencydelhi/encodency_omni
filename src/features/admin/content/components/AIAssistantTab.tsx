    ...
    <div className="grid items-start gap-3 xl:grid-cols-[minmax(0,1fr)_minmax(0,1fr)_320px]">
      <Card title="What would you like to create?">
        <div className="grid grid-cols-4 gap-1.5">
          {AI_TYPES.map((t) => (
            <button key={t.id} onClick={() => setSelected(t.id)} className={cn("rounded-sm border p-2 text-left transition", selected === t.id ? "border-purple-300 bg-purple-50 shadow-sm" : "border-[#E2E8F0] hover:border-[#CBD5E1]")}>
              <span className={cn("mb-1 grid size-7 place-items-center rounded-sm", selected === t.id ? "bg-[#7C3AED] text-white" : "bg-[#F1F5F9] text-[#64748B]")}>{t.icon}</span>
        ...
        <div className="mt-3">
          ...
          <textarea value={prompt} onChange={(e) => setPrompt(e.target.value)} rows={4} spellCheck className="w-full resize-none rounded-sm border border-[#D9E1EC] p-2.5 text-[12px] leading-5 outline-none focus:border-[#7C3AED]" />
        </div>
        ...
        <div className="grid grid-cols-3 gap-1.5">
          {[IMG.river, IMG.nature, IMG.water, IMG.people, IMG.lake, IMG.forest].map((src, i) => (
            <button key={i} className={cn("overflow-hidden rounded-sm border text-left", i === 0 ? "border-[#7C3AED] ring-2 ring-purple-100" : "border-[#E2E8F0]")}>
        ...
        <button onClick={() => setGenerated(true)} className="mt-3 flex h-10 w-full items-center justify-center gap-1.5 rounded-sm bg-[#7C3AED] text-[12.5px] font-semibold text-white shadow-sm transition hover:bg-[#6D28D9]">
      ...
      <Card title="AI result" subtitle="Review, then send to editor" action={<button className="text-[11px] font-semibold text-[#1769DF]">Regenerate</button>}>
        {generated ? (
          <>
            <div className="rounded-sm border border-[#E2E8F0] p-2.5 text-[12px] leading-5 text-[#33445F]">
              {MOCK_AI_CONTENT.caption}
              <br /><br />
              <span className="font-semibold text-[#1769DF]">{MOCK_AI_CONTENT.hashtags.join(" ")}</span>
            </div>
            <img src={IMG.people} alt="" className="mt-2 aspect-video w-full rounded-sm object-cover" />
            ...
            <div className="mt-2 flex gap-1.5">
              <button className="h-8 flex-1 rounded-sm border text-[11px] font-semibold text-[#687797]">Save draft</button>
              <button className="h-8 flex-1 rounded-sm bg-[#1769DF] text-[11px] font-semibold text-white">Use this post</button>
            </div>
          </>
        ) : (
          <div className="grid min-h-[200px] place-items-center rounded-sm border border-dashed border-[#CBD5E1] bg-[#F8FAFD] p-6 text-center">
        ...
        <Card title="Quick refinements">
          <div className="grid grid-cols-2 gap-1">
            {["Make shorter", "Add CTA", "Change tone", "More hashtags", "Hindi version", "Add emojis"].map((x) => (
              <button key={x} className="rounded-sm border border-[#E2E8F0] px-2 py-1.5 text-left text-[11px] font-semibold text-[#687797] hover:border-purple-200 hover:bg-purple-50/50">✨ {x}</button>
            ))}
          </div>
        </Card>
      </div>
