...
export function IdeasTab() {
  ...
  return (
    <div className="grid items-start gap-3 xl:grid-cols-[minmax(0,1fr)_320px]">
      <Card title="Content ideas" subtitle="Curated prompts for your next post" action={<button className="flex h-8 items-center gap-1.5 rounded-sm bg-purple-50 px-3 text-[11.5px] font-semibold text-purple-700 ring-1 ring-purple-200 hover:bg-purple-100"><Sparkles className="size-3.5" /> Generate with AI</button>}>
        <div className="grid gap-2 sm:grid-cols-2 2xl:grid-cols-3">
          {MOCK_IDEAS.map((idea, i) => (
            <button key={idea.id} onClick={() => setSelected(i)} className={cn("overflow-hidden rounded-sm border text-left transition", selected === i ? "border-[#1769DF] shadow-md ring-1 ring-blue-100" : "border-[#E2E8F0] hover:shadow-md")}>
      ...
      <Card title="Idea details" className="xl:sticky xl:top-4">
        ...
        <button className="mt-2.5 h-9 w-full rounded-sm bg-[#EB0711] text-[12px] font-semibold text-white transition hover:bg-[#D60811]">Use this idea</button>
        <button className="mt-1.5 h-8 w-full rounded-sm border text-[11.5px] font-semibold text-[#687797] hover:bg-slate-50">Customize with AI</button>
      </Card>
    </div>
  );
}