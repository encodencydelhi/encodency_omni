...
export function SelectField({ label, value, options, placeholder, required, onChange }: { label: string; value?: string; options?: string[]; placeholder?: string; required?: boolean; onChange?: (v: string) => void }) {
  return (
    <label className="min-w-0 flex-1">
      ...
      {options ? (
        <select value={value} onChange={e => onChange?.(e.target.value)} className="h-9 w-full appearance-none rounded-sm border border-[#D9E1EC] bg-white px-2.5 pr-8 text-[12.5px] font-medium text-[#24365A] outline-none transition hover:border-[#1769DF] focus:border-[#1769DF]">
          {placeholder && !value && <option value="">{placeholder}</option>}
          {options.map((o) => o && <option key={o} value={o}>{o}</option>)}
        </select>
      ) : (
        <span className="flex h-9 w-full items-center justify-between gap-2 rounded-sm border border-[#D9E1EC] bg-white px-2.5 text-[12.5px] font-medium text-[#24365A] transition hover:border-[#1769DF]">
...
export function Toggle({ on, onChange, label }: { on: boolean; onChange: () => void; label: string }) {
  return (
    <button role="switch" aria-checked={on} aria-label={label} onClick={onChange} className={cn("relative shrink-0 rounded-sm transition", on ? "bg-[#1769DF]" : "bg-[#D5DDE8]")} style={{ height: 20, width: 36 }}>
      <span className="absolute top-[2px] size-4 rounded-sm bg-white shadow-sm transition-all" style={{ [on ? "right" : "left"]: 2 } as React.CSSProperties} />
    </button>
  );
}
