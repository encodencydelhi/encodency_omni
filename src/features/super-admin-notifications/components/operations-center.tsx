...
function Compose({ snapshot, intents, setIntents, campaigns, setCampaigns }: { snapshot: NotificationSnapshot; intents: NotificationIntent[]; setIntents: (rows: NotificationIntent[]) => void; campaigns: Campaign[]; setCampaigns: (rows: Campaign[]) => void }) {
  ...
  const dirty = !saved && Boolean(title || subject || body);
  useEffect(() => {
...
function TemplateEditor({ template, templates, setTemplates }: { template: Template; templates: Template[]; setTemplates: (templates: Template[]) => void }) {
  ...
  const [saved, setSaved] = useState(false);
  const dirty = !saved && (subject !== (latest?.subject ?? "") || body !== (latest?.body ?? ""));
  useEffect(() => {
    if (!dirty) return undefined;
    const onBeforeUnload = (event: BeforeUnloadEvent) => { event.preventDefault(); event.returnValue = ""; };
    window.addEventListener("beforeunload", onBeforeUnload);
  ...
  return <Card><Title title="Template Editor" subtitle="Adds a new draft version; old versions remain unchanged." action={<button onClick={saveVersion} disabled={!subject || !body || invalidVariables.length > 0} className="rounded-md bg-[#111C3A] px-3 py-2 text-xs font-semibold text-white disabled:opacity-50">Save Template Version</button>} /><div className="grid gap-1 p-4 xl:grid-cols-[1fr_0.8fr]"><div className="space-y-3">{dirty ? <div className="rounded-md border border-amber-200 bg-amber-50 p-2 text-xs text-amber-800">Unsaved template edits are protected by the browser leave warning.</div> : null}<input value={subject} onChange={(e) => { setSaved(false); setSubject(e.target.value); }} className="w-full rounded-md border border-slate-200 px-3 py-2 text-sm" /><textarea value={body} onChange={(e) => { setSaved(false); setBody(e.target.value); }} className="min-h-28 w-full rounded-md border border-slate-200 px-3 py-2 text-sm" /><div className="rounded-md border border-slate-200 bg-slate-50 p-2 text-xs text-slate-600">Allowed variables: {template.variables.join(", ") || "None"}</div>{invalidVariables.length ? <div className="rounded-md border border-rose-200 bg-rose-50 p-2 text-xs text-rose-700">Invalid variables: {invalidVariables.join(", ")}</div> : null}{saved ? <div className="rounded-md border border-emerald-200 bg-emerald-50 p-2 text-xs text-emerald-700">New demo template version saved.</div> : null}</div><div className="rounded-md border border-slate-200 bg-slate-50 p-3"><p className="text-xs font-bold text-[#111C3A]">{subject}</p><p className="mt-2 text-sm text-slate-600">{body}</p><p className="mt-3 text-[11px] text-slate-500">Preview only. It does not mutate historic notifications.</p></div></div></Card>;
}
