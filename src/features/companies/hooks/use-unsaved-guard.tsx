...
/**
 * Protects unsaved work in drawers and dialogs.
 *
 * While `dirty`:
 ...
 *
 * Overlays are modal, so in-app link clicks cannot reach the page behind them;
 * the guard therefore only has to cover closing, Back and unload.
 */
export function useUnsavedGuard({
  ...
  dirty: boolean;
  /** Called when the user confirms leaving without saving, or when nothing was dirty. */
  onDiscard: () => void;
  /** Optional. Resolve `true` when the save succeeded and it is safe to leave. */
  ...
  useEffect(() => {
    if (!dirty) return;
    // A sentinel entry lets Back land on this page again so the prompt can appear.
