      ...
      <section className={cn(card, "overflow-hidden")}>
        <div className="flex flex-wrap items-center justify-between gap-2 border-b border-[#dde5ee] px-3 py-2.5">
          <div className="flex items-center gap-2">
            <Grid2X2 className="size-4 text-[#1877f2]" aria-hidden="true" />
            <h2 className="text-sm font-bold">Ad Sets</h2>
            <span className="rounded-full bg-[#eef2f7] px-2 py-0.5 text-[10px] font-bold text-[#475569]">
              {rows.length}
            </span>
            {values.campaign !== DEFAULTS.campaign && (
              <span className="text-[11px] text-[#64748b]">
                in <strong className="text-[#14213d]">{campaignFilter}</strong>
              </span>
            )}
          </div>
        ...
        {rows.length > 0 && (
          <div className="border-t border-[#dde5ee] px-3 py-2.5 text-[10px] text-[#64748b]">
            Showing {rows.length} of {adSets.length} ad sets
          </div>
        )}
      </section>
    </AdsWorkspace>
